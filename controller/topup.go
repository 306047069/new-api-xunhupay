...
func GetTopUpInfo(c *gin.Context) {
	...
	enableXunhupay := isXunhupayTopUpEnabled()
	if enableXunhupay {
		hasXunhupay := false
		for _, method := range payMethods {
			if method["type"] == model.PaymentMethodXunhupay {
				hasXunhupay = true
				break
			}
		}
		if !hasXunhupay {
			payMethods = append(payMethods, map[string]string{
				"name":      "微信支付",
				"type":      model.PaymentMethodXunhupay,
				"color":     "#07C160",
				"min_topup": strconv.Itoa(setting.XunhupayMinTopUp),
			})
		}
	}
	...
	data := gin.H{
		...
		"xunhupay_min_topup":      setting.XunhupayMinTopUp,
