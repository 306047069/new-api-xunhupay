...
const TopUp = () => {
  ...
  const [xunhupayMinTopUp, setXunhupayMinTopUp] = useState(1);
  const [xunhupayQrUrl, setXunhupayQrUrl] = useState('');
  const [xunhupayOrderId, setXunhupayOrderId] = useState('');
  const [xunhupayModalOpen, setXunhupayModalOpen] = useState(false);
  ...
  const requestAmountByPayment = async (payment, value) => {
    ...
    if (payment === 'xunhupay') {
      return getXunhupayAmount(value);
    }
  ...
  const preTopUp = async (payment) => {
    ...
    } else if (payment === 'xunhupay') {
      if (!enableXunhupayTopUp) {
        showError(t('管理员未开启虎皮椒支付充值！'));
        return;
      }
    } else {
  ...
  const xunhupayTopUp = async () => {
    const minTopUpValue = Number(xunhupayMinTopUp || 1);
    if (topUpCount < minTopUpValue) {
      showError(t('充值数量不能小于') + minTopUpValue);
      return;
    }
    ...
    setPaymentLoading(true);
    try {
      const res = await API.post('/api/user/xunhupay/pay', {
        amount: parseInt(topUpCount),
      });
      if (res !== undefined) {
        const { message, data } = res.data;
        if (message === 'success') {
          ...
          if (qrUrl) {
            setXunhupayQrUrl(qrUrl);
            setXunhupayOrderId(orderId);
            setXunhupayModalOpen(true);
          } else {
            showError(t('支付请求失败'));
          }
        } else {
  ...
  const getXunhupayAmount = async (value) => {
    if (value === undefined) {
      value = topUpCount;
    }
    setAmountLoading(true);
    try {
      // 虎皮椒没有单独的金额计算接口，直接返回充值数量
    ...
    try {
      ...
      if (success) {
        ...
        try {
          ...
          setXunhupayMinTopUp(data.xunhupay_min_topup || 1);
  ...
  return (
    <div className='w-full max-w-7xl mx-auto relative min-h-screen lg:min-h-0 mt-[60px] px-2'>
      ...
      <Modal
        title={t('微信支付')}
        visible={xunhupayModalOpen}
        onOk={() => setXunhupayModalOpen(false)}
        onCancel={() => setXunhupayModalOpen(false)}
        okText={t('关闭')}
        cancelText={t('复制支付链接')}
        footer={
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
            ...
            <Button
              type='primary'
              theme='solid'
              onClick={() => setXunhupayModalOpen(false)}
            >
              {t('关闭')}
            </Button>
          </div>
      ...
      >
        <div style={{ textAlign: 'center', padding: '16px 0' }}>
          <p>
            {t('订单号：')}
            <code>{xunhupayOrderId}</code>
          </p>
          <p style={{ marginBottom: 16 }}>
            {t('请使用微信扫一扫完成支付')}
          </p>
          <img
            src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(xunhupayQrUrl)}`}
            alt='wechat pay qr'
