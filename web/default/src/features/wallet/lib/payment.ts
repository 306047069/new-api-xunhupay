...
/**
 * Check if payment method is Xunhupay
 */
export function isXunhupayPayment(paymentType: string): boolean {
  return paymentType === PAYMENT_TYPES.XUNHUPAY
}
...
export function getDefaultPaymentType(topupInfo: TopupInfo | null): string {
  ...
  if (topupInfo.enable_xunhupay_topup) {
    return PAYMENT_TYPES.XUNHUPAY
  }
...
export function getMinTopupAmount(topupInfo: TopupInfo | null): number {
  ...
  if (topupInfo.enable_xunhupay_topup) {
    return topupInfo.xunhupay_min_topup || DEFAULT_MIN_TOPUP
  }
